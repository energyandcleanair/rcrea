...
meas_weather <- creadb::join_city_weather(meas,
...
ggplot(rbind(training_plot.rolled, predict_plot.rolled, measured_plot.rolled), aes(x=date)) +
  ...
  geom_vline(xintercept=as.POSIXct(training_prediction_cut), linetype="dotted", colour="blue") +
  ggtitle(paste("Rolling ", plotting_average_by_roll,"-",plotting_average_by, " average of ", pollutant, " level in ",city))
```

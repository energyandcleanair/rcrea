theme_crea_new <- function(
  ...
  grey2 = "grey70",
  grey3 = "grey95",
  strip_background = pal_crea[["Blue"]],
  strip_color = "white",
  ...
  theme(
    ...
    # Grid lines
    panel.grid.major = element_line(color = grey3),  # Lighter gray for major grid lines
    ...
    axis.text = element_text(color = grey1, size = fontsize4),

    # Facet strips (if you're using facets)
    strip.background = element_rect(fill = strip_background, color = NA),
    strip.text = element_text(color = strip_color, size = fontsize3, margin = margin(4, 0, 4, 0)),
    ...
    # Caption
    plot.caption = element_text(color = grey1, size = fontsize3, hjust = 0,
